...
const BillingForm = ({ dispatch, billing})=>{
    ...
    const initialTimeData = { time_type:'1', begin_time:moment('00','HH'), end_time:moment('00','HH'), fee_rate:0 } ;
    const finalFields = forEdit 
        ...
        prevItem.timeList.map(item=> {
            ...
            let obj = { ...item };
            obj['key'] = num;
            obj.begin_time = moment(obj.begin_time,'HH');
            obj.end_time = moment(obj.end_time, 'HH');
            return obj;
        })
        :
                                        ...
                                        fields.map((field, index)=>(
                                            <div key={field.key} className={style['field-container']}>
                                                ...
                                                <Form.Item labelCol={{span:24}} label="开始时间" >
                                                    <TimePicker value={field.begin_time} style={{width:'100%'}} format="HH"  allowClear={false} onChange={ value => changeFields(updateArr( value, fields, field, 'begin_time')) } />
                                                </Form.Item>
                                                <Form.Item labelCol={{span:24}} label="结束时间" >
                                                    <TimePicker value={field.end_time} locale={zhCN} style={{width:'100%'}} format="HH" allowClear={false} onChange={ value => changeFields(updateArr( value, fields, field, 'end_time')) }/>
                                                </Form.Item>
                                                <Form.Item labelCol={{span:24}} label="费率(元/kwh)">
                                                    <InputNumber min={0} style={{width:'100%'}} value={field.fee_rate} onChange={ value => changeFields(updateArr( value, fields, field, 'fee_rate')) } />
                                                </Form.Item>
                                                {
                                                    fields.length !== 1  
                                                    ?
